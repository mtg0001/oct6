...
// Simplified city list per state (major cities)
            ...
            <div className="mt-3 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <DateField
                  label="Data de Ida *"
                  value={dataIdaStr}
                  onChange={idaHandlers.onChange}
                  selected={dataIdaDate}
                  onSelect={idaHandlers.onSelect}
                  popoverOpen={dataIdaOpen}
                  setPopoverOpen={setDataIdaOpen}
                />
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <DateField
                  label="Data de Volta"
                  value={dataVoltaStr}
                  onChange={voltaHandlers.onChange}
                  selected={dataVoltaDate}
                  onSelect={voltaHandlers.onSelect}
                  popoverOpen={dataVoltaOpen}
                  setPopoverOpen={setDataVoltaOpen}
                />
                <div>
              ...
              {/* Origem UF + Origem Cidade + Destino UF + Destino Cidade */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div>
                  ...
                  <Select value={origemCidade} onValueChange={setOrigemCidade} disabled={!origemUF}>
                    <SelectTrigger className="mt-1">
                      <SelectValue placeholder={origemUF ? "Selecione..." : "Selecione o estado..."} />
                    </SelectTrigger>
                    <SelectContent>
                      {(CIDADES_POR_UF[origemUF] || []).map((c) => (
                ...
                <div>
                  ...
                  <Select value={destinoCidade} onValueChange={setDestinoCidade} disabled={!destinoUF}>
                    <SelectTrigger className="mt-1">
                      <SelectValue placeholder={destinoUF ? "Selecione..." : "Selecione o estado..."} />
                    </SelectTrigger>
                    <SelectContent>
                      {(CIDADES_POR_UF[destinoUF] || []).map((c) => (
                        <SelectItem key={c} value={c}>{c}</SelectItem>
                      ))}
                    </SelectContent>
                  </Select>
                ...
                <div key={p.id} className="border border-border rounded-md p-3 space-y-3">
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    ...
                    <div>
                      ...
                      <Input
                        value={p.cpf}
                        onChange={(e) => updatePassageiro(p.id, "cpf", e.target.value)}
                    ...
                    <div>
                      ...
                      <Input
                        value={p.rg}
                        onChange={(e) => updatePassageiro(p.id, "rg", e.target.value.replace(/\D/g, ""))}
          ...
          <fieldset className="border border-primary/30 rounded-md p-4">
            ...
            <div className="mt-3 space-y-4">
              <div>
                ...
                <div
                  className="mt-1 flex items-center gap-2 border border-input rounded-md px-3 py-2 cursor-pointer hover:border-primary transition-colors"
