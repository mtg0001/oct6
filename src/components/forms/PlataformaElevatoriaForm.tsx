...
const PlataformaElevatoriaForm = ({ open, onOpenChange, unidade }: PlataformaElevatoriaFormProps) => {
  ...
  const validate = (): boolean => {
    ...
    if (tiposSelecionados.length === 0) { toast({ title: "Selecione ao menos um tipo de plataforma", variant: "destructive" }); return false; }
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-3">
              {TIPOS_PLATAFORMA.map((tipo) => (
                <div key={tipo} className="flex items-center gap-2">
                  <Checkbox id={`plat-${tipo}`} checked={tiposSelecionados.includes(tipo)} onCheckedChange={() => toggleTipo(tipo)} />
                  <Label htmlFor={`plat-${tipo}`} className="text-sm cursor-pointer">{tipo}</Label>
                </div>
              ))}
            </div>
          </fieldset>
          ...
          <fieldset className="border border-primary/30 rounded-md p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-3">
              <DateField label="Data de entrega" value={dataEntrega} onChange={setDataEntrega} calOpen={entregaCalOpen} setCalOpen={setEntregaCalOpen} calDate={entregaCalDate} onCalSelect={(d) => handleCalendarSelect(d, setEntregaCalDate, setDataEntrega, setEntregaCalOpen)} />
              <DateField label="Data de retirada" value={dataRetirada} onChange={setDataRetirada} calOpen={retiradaCalOpen} setCalOpen={setRetiradaCalOpen} calDate={retiradaCalDate} onCalSelect={(d) => handleCalendarSelect(d, setRetiradaCalDate, setDataRetirada, setRetiradaCalOpen)} />
            </div>
          </fieldset>
