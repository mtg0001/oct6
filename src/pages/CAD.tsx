      ...
      <div className="space-y-3">
        ...
        {filtered.map((sol) => {
          ...
          return (
            <div
              key={sol.id}
              className="bg-card border rounded-lg p-4 flex flex-wrap items-center gap-4 shadow-sm"
              style={{ borderColor: "hsl(330, 70%, 85%)" }}
            >
              <div
                className="h-10 w-10 rounded-full flex items-center justify-center shrink-0"
                style={{ backgroundColor: "hsl(330, 70%, 93%)", color: "hsl(330, 70%, 45%)" }}
              >
              ...
              <div className="flex-1 min-w-0 grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm">
                ...
                <div>
                  ...
                  <Badge variant="outline">{sol.unidade === "goiania" ? "GO" : sol.unidade === "mairipora" ? "MA" : "SP"}</Badge>
                </div>
                <div>
              ...
              <Button
                size="sm"
                onClick={() => navigate(`/cad/${filtro || "pendentes"}/solicitacao/${sol.id}`)}
                style={{ backgroundColor: "hsl(330, 70%, 55%)", color: "white" }}
