...
import {
  ...
  ensurePJLoaded, getPJ, subscribePJ, addPJ, updatePJ, deletePJ,
} from "@/stores/colaboradoresStore";

// Local UI row extends DB row with editing state
...
const ColaboradoresPJ = () => {
  ...
  useEffect(() => {
    ensurePJLoaded().then(() => {
      setRows(getPJ().map(c => ({ ...c, isEditing: false, isNew: false })));
      setLoading(false);
    });
    return subscribePJ(() => {
      setRows(prev => {
        const dbRows = getPJ();
        // Merge: keep editing rows as-is, update non-editing rows from DB
        const editingIds = new Set(prev.filter(r => r.isEditing).map(r => r.id));
        const newRowIds = new Set(prev.filter(r => r.isNew).map(r => r.id));
        const merged: UIRow[] = [];
  ...
  const saveRow = async (id: string) => {
    ...
    try {
      if (row.isNew) {
        const created = await addPJ(row.nome, row.dados);
        setRows(prev => prev.filter(r => r.id !== id).concat({ ...created, isEditing: false, isNew: false }));
      } else {
        await updatePJ(id, { nome: row.nome, dados: row.dados });
        setRows(prev => prev.map(r =>
  ...
  const changeStatus = async (id: string, newStatus: RowStatus) => {
    ...
    try {
      await updatePJ(id, { status: newStatus });
              ...
              {sorted.map((c) => {
                ...
                return (
                  <tr key={c.id} className={`border-t border-border hover:bg-muted/30 transition-colors ${isInativo ? "bg-destructive/5" : isPendenteTI ? "bg-orange-500/5" : editable ? "bg-[hsl(var(--sidebar-primary))]/5" : ""}`}>
                    <td className="sticky left-0 z-10 px-2 py-2 border-r border-border w-[100px] min-w-[100px] max-w-[100px]" style={{ backgroundColor: 'hsl(var(--card))' }}>
                      {editable ? (
